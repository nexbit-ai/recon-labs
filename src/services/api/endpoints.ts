...
export const transactionsAPI = {
  ...
  // Get total transactions counts summary across all tabs
...
export const monthOnMonthGrowthAPI = {
  getMonthOnMonthGrowth: (params: { platform: 'amazon' | 'flipkart' | 'd2c' | 'other' | 'amazon_uk' | 'myntra'; start_date: string; end_date: string }) =>
    apiService.get<any>(API_CONFIG.ENDPOINTS.MONTH_ON_MONTH_GROWTH, { ...params, date_field: params.platform === 'd2c' ? 'order_date' : 'settlement_date' }),
};
...
export const amazonAuthAPI = {
  ...
  getStatus: () =>
    apiService.get<{ connected: boolean; client_id?: string; application_id?: string; marketplace_regions?: string[] }>(API_CONFIG.ENDPOINTS.AMAZON_STATUS),
