...
const AmazonCallback: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!code) {
      setStatus('error');
      setError('Missing authorization code from Amazon.');
      return;
    }
    ...
    const completeAuth = async () => {
      ...
      try {
        ...
        const response = await api.amazonAuth.callback(code, state || '');
        if (response.statusCode === 200) {
          setStatus('success');
          setTimeout(() => {
            navigate('/integrations', { state: { success: `Successfully connected to Amazon Seller Central` } });
          }, 2000);
        } else {
          throw new Error('Failed to complete authentication');
        }
      } catch (err: any) {
        console.error('Amazon Auth Callback Error:', err);
        setStatus('error');
        setError(err?.response?.data?.error || 'Failed to connect Amazon store.');
      }
    };
  ...
  return (
    ...
    >
      ...
      >
        {status === 'loading' && (
          <>
            <CircularProgress size={60} sx={{ mb: 4, color: '#f59e42' }} />
            <Typography variant="h5" sx={{ fontWeight: 800, mb: 2 }}>
              Connecting Amazon...
            </Typography>
            <Typography variant="body2" color="text.secondary">
              Please wait while we securely finalize your Amazon connection.
            </Typography>
          </>
        )}
