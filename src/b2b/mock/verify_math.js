import { headline, channelPerformance, reconLineItems } from './index.js';
import {
  blinkitSettlements,
  blinkitDeductionRecords,
  blinkitCreditDebitNotes
} from './blinkitRecon.js';
...
const heroOther = heroSet.components.find(c => c.label === 'Other adjustments').amount; // this is negative in data
// Settlement expected = revenue - ded1 - ded2 ... + additions
// In data, Other is -3522 (deduction of -3522 means addition)
// let's just sum all components (taking care of sign)
...
// CNDN is absolute sum of CreditDebitNotes impact
const cndnImpact = blinkitCreditDebitNotes.reduce((acc, cn) => acc + Math.abs(cn.settlementImpact), 0);
// Wait, CNDN in settlement components is 12750. Let's check records
